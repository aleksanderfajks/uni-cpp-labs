...
int main() {
	...
	//Zadanie 3 - testy kopiowania danych o zadanym rozmiarze
	int temp[3]={4,7,9};
	int *destination=get_left_fence_address(&memory_sector)+1;	//Miejsce docelowe kopiowania to początek tablicy pamięci
	assert(copy_memory(&memory_sector, destination, temp, 3)==NO_ERROR);
	assert(read_memory(&memory_sector, destination, &buffer)==NO_ERROR);
	assert(read_memory(&memory_sector, destination+1, &buffer)==NO_ERROR);
	assert(read_memory(&memory_sector, destination+2, &buffer)==NO_ERROR);
